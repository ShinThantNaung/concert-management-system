...
beforeAll(async () => {
    const load = async <T>(label: string, importer: () => Promise<T>): Promise<T> => {
        try {
    ...
    const ticketModule = await load("Ticket", () => import("../src/entity/Ticket.ts"));
    const userModule = await load("User", () => import("../src/entity/User.ts"));
    const routesModule = await load("routes", () => import("../src/routes.ts"));
    const servicesModule = await load("services", () => import("../src/services.ts"));
...
describe("concert APIs", () => {
    ...
    it("GET /api/concerts/:id returns a concert", async () => {
        ...
        const response = await request(app).get(`/api/concerts/${concert.concertId}`);
...
describe("reservation and purchase", () => {
    ...
    it("POST /api/purchase completes a reservation without extra stock decrement", async () => {
        ...
        expect(user?.status).toBe("COMPLETET");
    });
